...
`HashMap` je jedna z nejpoužívanějších kolekcí v Javě. Slouží k ukládání dvojic **klíč → hodnota**,
kde každý klíč je
...
`HashMap` využívá **hashovací tabulku**, což je pole tzv. **bucketů** (segmentů). Každý bucket může
obsahovat více
