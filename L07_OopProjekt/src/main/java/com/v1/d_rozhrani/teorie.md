...
interface v Javě je smlouva (kontrakt), která říká:

    „Kdo mě implementuje, musí napsat tyto metody.“

Rozdíl oproti class je, že interface neobsahuje implementaci (většinou) – jen předpis toho, co má
třída umět.
...
1) Umožňuje jednotné chování různých tříd (např. všechny třídy, které „se dají uložit“, implementují
   Savable)
2) Pomáhá k volné vazbě mezi částmi systému (dobrá architektura)
3) Umožňuje polymorfismus – různé objekty se chovají stejně z pohledu toho, kdo je používá
4) **Flexibilita a rozšiřitelnost**: Umožňuje změnit nebo přidat nové funkce bez změny existujícího
   kódu.
